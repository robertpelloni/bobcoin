...
- [x] **WebRTC P2P Multiplayer:** Implemented a full WebSocket signaling server in the legacy Node signaling service and `simple-peer` WebRTC client in `RhythmGame.jsx`. Players can find matches, establish direct peer-to-peer connections, and stream live score updates without any central relay while the broader HTTP compatibility surface continues migrating to Go.
