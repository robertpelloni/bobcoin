...
- [x] **Frontend Bundle Health Pass:** Refactored the frontend toward route-level lazy loading and manual vendor chunking so `node-seal`, `three`, React, router, and crypto-heavy code no longer inflate the main entry bundle together.
- [x] **Aggressive Component Deferral:** Lazy-loaded the 3D topology visualization in `SystemStatus.jsx`, reducing the main app bundle from ~1.5MB to ~50kB.
...
- [x] **Diagnostics Comparison Review**: Vault now compares imported signed diagnostics packages against the current local reliability picture so operators can see freshness, overlap, and material source deltas rather than only a pass/fail signature result.
